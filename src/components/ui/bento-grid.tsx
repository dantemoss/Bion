  ...
  >
    ...
    <div className={compact ? "p-3 sm:p-2" : "p-4"}>
      <div className={cn(
        ...
        <p className={cn(
          "text-neutral-400 line-clamp-1",
          compact ? "text-xs max-w-full" : "max-w-lg"
        )}>{description}</p>
      </div>
      ...
      >
        <Button
          ...
          asChild
          size="sm"
          className={cn("pointer-events-auto p-0", compact && "text-xs h-auto")}
        >
          <a href={href}>
            {cta}
    ...
    >
      ...
      >
        <a href={href}>
          {cta}
          <ArrowRightIcon className={cn("rtl:rotate-180", compact ? "ms-1 h-3 w-3" : "ms-2 h-4 w-4")} />
        </a>
      </Button>
    </div>
